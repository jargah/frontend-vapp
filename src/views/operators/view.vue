    ...
    <v-container fluid class="py-6">
        <div class="d-flex align-center justify-space-between mb-4 ga-3">
            <div class="d-flex align-center ga-3">
                ...
                <h1 class="text-h5 mb-0">Detalle Operador #{{ id }}</h1>
            </div>
            <v-btn color="primary" prepend-icon="mdi-pencil-outline"
                :to="{ name: 'operators-edit', params: { id } }">Editar</v-btn>
        </div>
            ...
            <template v-else>
                ...
                <v-card-text>
                    <v-row>
                        ...
                        <v-col cols="12">
                            <v-sheet class="pa-4 rounded-lg border">
                                <div class="text-overline mb-2">Vehículo asignado</div>
                                <v-row>
                                    <v-col cols="12" md="3"><strong>Placas:</strong> {{ item.vehicle.plates }}</v-col>
                                    <v-col cols="12" md="3"><strong>Color:</strong> {{ item.vehicle.color }}</v-col>
                                    <v-col cols="12" md="3"><strong>Año:</strong> {{ item.vehicle.year }}</v-col>
                                    <v-col cols="12" md="3"><strong>Kilometraje:</strong> {{
                                        item.vehicle.mileage.toLocaleString() }} km</v-col>
                                </v-row>
                            </v-sheet>
                        </v-col>
                    </v-row>
                ...
                <v-card-actions class="justify-end">
                    ...
                    <v-btn color="primary" :to="{ name: 'operators-edit', params: { id } }"
...
async function load() {
    try {
        loading.value = true
        error.value = null
        item.value = await OperatorsService.getById(id.value)
