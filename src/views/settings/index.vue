<!-- src/views/configs/list.vue -->
<template>
    <v-container fluid class="py-6">
        <div class="d-flex align-center justify-space-between mb-4 ga-3">
            <h1 class="text-h5 mb-0">Configuraciones</h1>
            <v-btn color="primary" prepend-icon="mdi-plus" :to="{ name: 'settings-add' }">Agregar</v-btn>
        </div>
            ...
            <v-data-table :headers="headers" :items="filtered" :items-per-page="itemsPerPage" :page="page"
                ...
                <template #item.actions="{ item }">
                    <div class="d-flex ga-1">
                        <v-btn :to="{ name: 'settings-view', params: { id: item.id } }" icon="mdi-eye-outline"
                            variant="text" :title="`Ver #${item.id}`" />
                        <v-btn :to="{ name: 'settings-edit', params: { id: item.id } }" icon="mdi-pencil-outline"
                            variant="text" :title="`Editar #${item.id}`" />
                        <v-btn icon="mdi-trash-can-outline" variant="text" color="error" :title="`Eliminar #${item.id}`"
        ...
        <v-dialog v-model="deleteDialog" max-width="420" transition="dialog-bottom-transition">
            <v-card rounded="xl">
                ...
                <v-divider />
                <v-card-text>
                    ¿Eliminar configuración <strong>#{{ pendingItem?.id }}</strong> (<em>{{ pendingItem?.name }}</em>)?
                </v-card-text>
...
const headers = [
    ...
    { title: 'Creación', key: 'created_at', width: 190 },
...
onMounted(async () => {
    items.value = await ConfigsService.list()
})
...
const filtered = computed(() => {
    ...
    return items.value.filter(i =>
        [i.id, i.name, i.value, i.status, formatDate(i.created_at)]
            .join(' ')
            .toLowerCase()
            .includes(q)
    )
...
const pendingItem = ref<ConfigItem | null>(null)
function openDelete(item: ConfigItem) { pendingItem.value = item; deleteDialog.value = true }
function closeDelete() { deleteDialog.value = false; pendingItem.value = null }
async function confirmDelete() {
    if (!pendingItem.value) return
    await ConfigsService.remove(pendingItem.value.id)
    items.value = await ConfigsService.list()
    closeDelete()
    snackbar.msg = 'Configuración eliminada.'
    snackbar.open = true
}
