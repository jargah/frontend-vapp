<!-- src/views/typeMotor/add.vue (edición de Configuración con { name }) -->
<template>
    <v-container fluid class="py-6">
        <div class="d-flex align-center justify-space-between mb-4 ga-3">
            <div class="d-flex align-center ga-3">
                ...
                <h1 class="text-h5 mb-0">Editar Tipo Motor</h1>
            </div>
        </div>
...
const view = computed(() => store.getters['typeMotor/view'])

// Cargar la config si hay id
async function load() {
    if (!Number.isNaN(id.value)) {
        await store.dispatch('typeMotor/view', id.value)
    }
}
...
const onSubmit = handleSubmit(
    async (values) => {
        try {
            ...
            // values === { name: '...' }
            const result = await store.dispatch('typeMotor/edit', { id: id.value, body: values })
...
function goBack() {
    ...
    else router.push({ name: 'type-motor-list' })
}
