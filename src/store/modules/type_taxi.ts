...
export const typeTaxi: Module<TypePaymentState, any> = {
    namespaced: true,
    state: () => ({
        prospects: null,
        table: {
    ...
    actions: {
        ...
        async create({ commit, rootGetters }, payload) {
            ...
            const result = await postJson(
                'administrator/type-taxi/create', 
                payload
            );
        ...
        async view({ commit, rootGetters }, payload: number) {
            ...
            const result = await getJson(
                `administrator/type-taxi/${payload}/view`
            );
        ...
        async edit({ commit, rootGetters }, payload) {
            ...
            const result = await putJson(
                `administrator/type-taxi/${payload.id}/edit`, 
                payload.body
            );
