...
export const normalRates: Module<NormalRatesState, any> = {
    namespaced: true,
    state: () => ({
        rate: null
    }),
    ...
    actions: {
        async list({ commit, rootGetters }) {
            ...
            const result = await getJson('/administrator/configuration/normal-rates')
        ...
        async edit({ commit, rootGetters }, payload) {
            ...
            const result = await putJson('/administrator/configuration/normal-rates', payload)
