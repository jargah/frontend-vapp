...
export const vehicles: Module<VehiclesState, any> = {
    namespaced: true,
    state: () => ({
        prospects: null,
        table: {
    ...
    actions: {
        ...
        async view({ commit, rootGetters }, payload: number) {
            ...
            const result = await getJson(
                `administrator/vehicles/${payload}/view`
            );
        ...
        async edit({ commit, rootGetters }, payload) {
            ...
            const result = await putJson(
                `administrator/vehicles/${payload.id}/edit`, 
                payload.body
            );
