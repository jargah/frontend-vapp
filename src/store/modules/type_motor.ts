...
export type TypeMotorState = {
    view?: any,
    table: DatatableParams
}

export const typeMotor: Module<TypeMotorState, any> = {
    namespaced: true,
    state: () => ({
        prospects: null,
        table: {
    ...
    actions: {
        ...
        async create({ commit, rootGetters }, payload) {
            ...
            const result = await postJson(
                'administrator/type-motor/create', 
                payload
            );
        ...
        async view({ commit, rootGetters }, payload: number) {
            ...
            const result = await getJson(
                `administrator/type-motor/${payload}/view`
            );
        ...
        async edit({ commit, rootGetters }, payload) {
            ...
            const result = await putJson(
                `administrator/type-motor/${payload.id}/edit`, 
                payload.body
            );
