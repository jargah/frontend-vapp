...
export type TypePaymentState = {
    view?: any,
    table: DatatableParams
}

export const typePayment: Module<TypePaymentState, any> = {
    namespaced: true,
    state: () => ({
        prospects: null,
        table: {
    ...
    actions: {
        async list({ commit, rootGetters }, payload: DatatableParams) {
            ...
            const result = await getJson(
                'administrator/type-payment/list', {
        ...
        async create({ commit, rootGetters }, payload) {
            ...
            const result = await postJson(
                'administrator/type-payment/create', 
                payload
            );
        ...
        async view({ commit, rootGetters }, payload: number) {
            ...
            const result = await getJson(
                `administrator/type-payment/${payload}/view`
            );
        ...
        async edit({ commit, rootGetters }, payload) {
            ...
            const result = await putJson(
                `administrator/type-payment/${payload.id}/edit`, 
                payload.body
            );
